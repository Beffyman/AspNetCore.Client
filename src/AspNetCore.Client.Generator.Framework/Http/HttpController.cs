...
namespace AspNetCore.Client.Generator.Framework.Http
{
	/// <summary>
	/// Information about a group of endpoints used for generation
	/// </summary>
	public class HttpController : IResponseTypes, IHeaders, IIgnored, INamespaceSuffix, IObsolete, INavNode, IAuthorize
	{
		/// <summary>
		...
		/// </summary>
		public HttpController BaseController { get; set; }
		...
		/// </summary>
		public IList<HttpEndpoint> Endpoints { get; set; } = new List<HttpEndpoint>();
		...
		/// </summary>
		/// <returns></returns>
		public IEnumerable<HttpEndpoint> GetEndpoints()
		{
			var resolvedEndpoints = Endpoints.Union(BaseController?.GetEndpoints() ?? new List<HttpEndpoint>())
											.ToList();
